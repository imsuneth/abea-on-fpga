...
inline ulong get_kmer_rank(__global char *str, ulong k) {
  // uint32_t p = 1;
  // printf("->%c ", *str);
  ulong r = 0;
...
align_kernel_single(__global AlignedPair *restrict out_2,
                    __global int32_t *restrict n_event_align_pairs,
                    __global char *restrict sequence, int32_t sequence_len,
                    __global event1_t *restrict events, ulong n_events1,
                    __global scalings_t *restrict scaling_ptr,
                    __global model_t *restrict models,
                    __global ulong *restrict kmer_ranks,
                    __global float *restrict bands,
                    __global uint8_t *restrict trace,
                    __global EventKmerPair *restrict band_lower_left) {

  scalings_t scaling = *scaling_ptr;
  ...
  // // printf("sequence_len %d", sequence_len);
