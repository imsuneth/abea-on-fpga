...
#define BAND_ARRAY_SHM(r, c) (bands_shm[r * ALN_BANDWIDTH + c])
//******************************************************************************************************
/*core kernel 1*/
//******************************************************************************************************
...
align_kernel_core_1(
    ...
    __global float *restrict bands_shm,
    __global EventKmerPair *restrict band_lower_left_shm) {
...
align_kernel_core_4(
  ...
  // __syncthreads();
  // printf("IN CORE KERNEL - IN LOOP - before barrier 4n");
  // barrier(CLK_LOCAL_MEM_FENCE); // OpenCL
  ...
  //   int32_t sequence_len = read_len[i];
  //   int3t n_event = n_events1[i];

  //   int32_t n_events = n_event;
  //   int_t n_kmers = sequence_len - KMER_SIZE + 1;

  //   // dp matrix
  // int32_t n_rows = n_events + 1;
  //   int32_t n_cols = n_kmers + 1;
  //  nt32_t n_bands = n_rows + n_cols;
  //   barrr(CLK_LOCAL_M_FENCE);
  //   for (int32_t band_idx = 2; band_idx < n_bands; ++band_idx) {
  //     barrier(CLK_LOL_MEM_FENCE);
