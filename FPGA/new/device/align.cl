...
__global inline float log_probability_match_r9(scalings_t scaling,
...
EventKmerPair move_down(EventKmerPair curr_band){
    __private EventKmerPair ret =  {curr_band.event_idx + 1, curr_band.kmer_idx };
    return ret;
}
EventKmerPair move_right(EventKmerPair curr_band){
    __private EventKmerPair ret = { curr_band.event_idx, curr_band.kmer_idx + 1 };
    return ret;
}
...
__kernel void align_kernel_pre_2d(
    ...
    if (i < n_bam_rec) {
      ...
      __global float *bands = &bands1[(read_ptr[i]+event_ptr[i])*ALN_BANDWIDTH];
      __global uint8_t *trace = &trace1[(read_ptr[i]+event_ptr[i])*ALN_BANDWIDTH];
      __global EventKmerPair* band_lower_left = &band_lower_left1[read_ptr[i]+event_ptr[i]];
...
__kernel void align_kernel_core_2d_shm(
    ...
    __global int32_t* restrict n_events1,
    __global ptr_t* restrict event_ptr,
    __global scalings_t* restrict scalings, 
    int32_t n_bam_rec,
    __global model_t* restrict model_kmer_caches,
    __global float * restrict band,
    ...
    if (i < n_bam_rec && offset<ALN_BANDWIDTH) {
        ...
        __global uint8_t *trace = &traces[(read_ptr[i]+event_ptr[i])*ALN_BANDWIDTH];
        __global EventKmerPair* band_lower_left = &band_lower_lefts[read_ptr[i]+event_ptr[i]];;
