...
void align_ocl(core_t *core, db_t *db)
{
  ...
  if (core->opt.verbosity > 1)
    fprintf(stderr, "Pre kernel done: %.3f seconds\n", (realtime() - realtime1));
  ...
  cl_mem kmer_rank = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sum_read_len * sizeof(size_t), kmer_rank_host, &status);
  checkError(status, "Failed clCreateBuffer");
  ...
  cl_mem bands = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(float) * sum_n_bands * ALN_BANDWIDTH, bands_host, &status);
  checkError(status, "Failed clCreateBuffer");
  ...
  cl_mem band_lower_left = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sum_n_bands * sizeof(EventKmerPair), band_lower_left_host, &status);
  checkError(status, "Failed clCreateBuffer");
  ...
  status = clSetKernelArg(align_kernel_single, 0, sizeof(cl_mem), &read_len);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 1, sizeof(cl_mem), &read_ptr);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 2, sizeof(cl_mem), &event_table);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 3, sizeof(cl_mem), &n_events);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 4, sizeof(cl_mem), &event_ptr);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 5, sizeof(cl_mem), &scalings);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 6, sizeof(cl_mem), &model);
  checkError(status, "Failed to set kernel args");

  status = clSetKernelArg(align_kernel_single, 7, sizeof(int32_t), &n_bam_rec);
  checkError(status, "Failed to set kernel args");
  ...
  if (core->opt.verbosity > 1)
    fprintf(stderr, "Core kernel done: %.3f seconds\n", (realtime() - realtime1));
  realtime1 = realtime();
