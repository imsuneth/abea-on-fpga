...
static cl_kernel align_kernel_pre_core = NULL;
...
void align_ocl(core_t *core, db_t *db)
{
  ...
  status = clSetKernelArg(align_kernel_pre_core, 0, sizeof(cl_mem), &read);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");

  status = clSetKernelArg(align_kernel_pre_core, 1, sizeof(cl_mem), &read_len);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 2, sizeof(cl_mem), &read_ptr);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 3, sizeof(cl_mem), &n_events);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 4, sizeof(cl_mem), &event_ptr);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 5, sizeof(cl_mem), &model);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 6, sizeof(int32_t), &n_bam_rec);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 7, sizeof(cl_mem), &model_kmer_cache);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 8, sizeof(cl_mem), &bands);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 9, sizeof(cl_mem), &trace);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 10, sizeof(cl_mem), &band_lower_left);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");

  status = clSetKernelArg(align_kernel_pre_core, 11, sizeof(cl_mem), &event_table);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  status = clSetKernelArg(align_kernel_pre_core, 12, sizeof(cl_mem), &scalings);
  checkError(status, "Failed to set kernel args to align_kernel_pre_core");
  ...
  clEnqueueNDRangeKernel(queue, align_kernel_pre_core, 2, NULL, gridpre, blockpre, 0, NULL, &event);
...
// Create the program.
...
#else
  std::string binary_file = getBoardBinaryFile("align_2k", device);
  printf("Using AOCX: %s\n", binary_file.c_str());
  ...
  const char *kernel1_name = "align_kernel_pre_core"; // Kernel name, as defined in the CL file
  const char *kernel3_name = "align_kernel_post";     // Kernel name, as defined in the CL file

#ifdef SEPARATE_KERNELS
  align_kernel_pre_core = clCreateKernel(program1, kernel1_name, &status);
...
#else
  align_kernel_pre_core = clCreateKernel(program, kernel1_name, &status);
  checkError(status, "Failed to pre create kernel");
...
void cleanup()
{
  ...
  if (align_kernel_pre_core)
  {
    clReleaseKernel(align_kernel_pre_core);
  }
