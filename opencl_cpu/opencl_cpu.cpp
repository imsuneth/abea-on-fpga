                      ...
                      struct twoArrays *arg1)
{
    ...
    memObjects[0] = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                   sizeof(struct twoArrays), arg1, NULL);
...
int main(int argc, char **argv)
{
    ...
    program = CreateProgram(context, device, "align.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return 1;
    ...
    kernel = clCreateKernel(program, "align", NULL);
    if (kernel == NULL)
    {
    ...
    // float result[ARRAY_SIZE];(struct twoArrays *)
